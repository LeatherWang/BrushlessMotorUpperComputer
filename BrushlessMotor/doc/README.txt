vector 优点是动态分配内存, 缺点是损失效率, 空间效率损失比较大, 速度在优化充分时接近原始数组

命名规范：（2017-11-04）
对象/控件名称：类型驼峰法+下划线+名称驼峰，例，QLabel label_serialNum;
函数名称：驼峰法，例，addTwoNumber();
普通变量：驼峰法，例，int currentStep;
特殊变量：前面加属性，名称驼峰法，flag_configured
